...
df = pd.DataFrame()
data = []
l = 'ID\tGENE\tACC\tTM protein\tSignalp\tTotal_MUT\tUniProt_MUT\tClinVar_MUT\tCOSMIC_MUT\tPosition\tSignalP_TM\tSignalP_noTM\tN_terminus\tTopology\tTopfind\n'
for id in dic:
    ...
    if dic[id].signal in [0, 1]:
        ...
        l += id+'\t'+ dic[id].gene+'\t'+ dic[id].acc+'\t'+ dic[id].kw+'\t'+ str(dic[id].signal) + '\t' + str(len(list(set(dic[id].mut)))) + '\t' + str(len(dic[id].uniprot_mut))+'\t'+ str(len(dic[id].clinvar_mut))+'\t'+ str(len(dic[id].cosmic_mut))+'\t'+ str(dic[id].pos_notm) + '\t' + str(dic[id].prob_tm)+'\t'+ str(dic[id].prob_notm)+'\t'+ dic[id].type+'\t'+ dic[id].topo+'\t'+ ','.join(dic[id].topfind)+'\n'
        row.append(id)
        ...
        data.append(row)
#sys.exit()
df = pd.DataFrame(data, columns=['ID', 'GENE', 'ACC', 'TM protein', 'Signalp', 'Total_MUT', 'UniProt_MUT', 'ClinVar_MUT', 'COSMIC_MUT', 'Position', 'SignalP_TM', 'SignalP_noTM', 'N_terminus', 'Topology', 'Topfind'])
print (df.shape)
fig=pylab.figure()
