## Script to generate and plot Internal TMDs
## analysis data
...
map_start = {}
for line in open('../data/part_c_'+species+'_annotations.tsv', 'r'):
    if line[0] != '#':
        name = line.split('\t')[0]
...
if species == 'human':
    SIGNALp = '../data/part_c_signalp/'
else:
    SIGNALp = '../data/part_c_signalp_caeel/'
...
l = 'ID\tGENE\tACC\tUniProt-KB\tTM protein\tTotal_MUT\tUniProt MUT\tClinVar MUT\tCOSMIC MUT\tPosition\tSignalP_TM\tSignalP_noTM\tN-terminus\tTopology\tTopfind\tTrypsinCleavageSite\tTrypsinPepLength\tTrypsinPeptide\tLegumainCleavageSite\tLegumainPepLength\tLegumainPeptide\tGluCCleavageSite\tGluCPepLength\tGluCPeptide\n'
total_count = 0
count_excl = 0
count_type1 = 0
count_type2 = 0
...
for id in dic:
    if dic[id].signal in [0, 1]:
        ...
        for tm in dic[id].transmem:
            ...
            prob_tm = 0.0
            prob_notm = 0.0
            if os.path.isfile(SIGNALp+name+'_long_noTM.out'):
                #print ('YES')
                for line in open(SIGNALp+name+'_long_noTM.out', 'r'):
                ...
                for line in open(SIGNALp+name+'_long_TM.out', 'r'):
                    if line.split()[0] == 'max.' and line.split()[1]=='Y':
                ...
                print ('No', SIGNALp+name+'_long_noTM.out')
                #sys.exit()
...
print (cleavedSeq)
open('part_c_cleavedSeq.fasta', 'w').write(cleavedSeq)
#sys.exit()
df = pd.DataFrame(data, columns=['ID', 'GENE', 'ACC', 'UniProtKB', 'Name', 'TMD', 'Start', 'End', 'Position', 'TM protein', 'Signalp', 'Total_MUT', 'UniProt_MUT', 'ClinVar_MUT', 'COSMIC_MUT', 'Size', 'N_terminus', 'SignalP_TM', 'SignalP_noTM', 'Topfind', 'Highlight', 'TrypsinCleavageSite', 'TrypsinPepLength', 'TrypsinPeptide', 'LegumainCleavageSite','LegumainPepLength', 'LegumainPeptide', 'GluCCleavageSite', 'GluCPepLength', 'GluCPeptide'])
...
#df.to_csv('../data/part_c_data.tsv', sep='\t', index=False)

if species != 'human':
    df.to_csv('../data/'+species+'/part_c_data.tsv', sep='\t', index=False)
else:
    df.to_csv('../data/part_c_data.tsv', sep='\t', index=False)
