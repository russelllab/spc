...
for line in gzip.open('../data/uniprot_2020-05_features_Hsa.tsv.gz', 'rt'):
    if line.split('\t')[1] == 'VARIANT' and '->' in line.split('\t')[5]:
        ...
        if id in dic:
            #mut = line.split('\t')[0].split('/')[1].split()[0]
            print (line)
            mut = line.split('\t')[5].split('-')[0]
            mut += str(line.split('\t')[4])
            mut += line.split('\t')[5].split('>')[1]
            if 'in'  in line:
                #print (line.split('\t')[3].split('(')[1])
                #dis = line.split('\t')[3].split('(')[1]
                #if len(dis.split()) >1:
...
l = 'ID\tGENE\tACC\tTM protein\tSignal peptide\tUniProt_MUT\tClinVar_MUT\tCOSMIC_MUT\tSub-cellular location\tPosition\tSignalP_TM\tSignalP_noTM\tN-terminus\tTopology\n'
for id in dic:
    ...
    if dic[id].signal in [0, 1]:
        ...
        l += id+'\t'+dic[id].gene+'\t'+dic[id].acc+'\t'+dic[id].kw+'\t'+str(dic_sp[dic[id].signal])+'\t'+str(dic[id].mut)+'\t'+str(dic[id].clinvar_mut)+'\t'+str(dic[id].cosmic_mut)+'\t'+str(dic[id].subcell)+'\t'+str(dic[id].pos_notm)+'\t'+str(dic[id].prob_tm)+'\t'+str(dic[id].prob_notm)+'\t'+dic[id].type+'\t'+dic[id].topo+'\n'
        row.append(id)
        row.append(dic[id].kw)
        row.append(dic[id].signal)
        row.append(dic[id].mut)
        row.append(dic[id].clinvar_mut)
        row.append(dic[id].cosmic_mut)
...
#print (data)
df = pd.DataFrame(data, columns=['ID', 'TM protein', 'SIGNALp', 'Uniprot_MUT', 'ClinVar_MUT', 'COSMIC_MUT', 'N-terminus', 'signalp_TM', 'signalp_noTM', 'Categories'])
...
colors = ['teal', 'pink', 'purple', 'magenta', 'red', 'green', 'blue', 'brown']
ax = sns.scatterplot(data=df, palette=colors, x="signalp_noTM", y="signalp_TM", hue="Categories", hue_order=ho, style='TM protein', legend='brief', style_order=['YES', 'NO'])
...
#plt.show()
sys.exit()
plt.savefig('../plots/customized.svg')
plt.savefig('../plots/customized.eps')
plt.savefig('../plots/customized.png')
#plt.savefig('../plots/customized_without_signal_peptide.svg')
#plt.savefig('../plots/customized_without_signal_peptide.eps')
#plt.savefig('../plots/customized_without_signal_peptide.png')
