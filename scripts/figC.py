    ...
    if dic[id].signal in [0, 1]:
        ...
        for tm in dic[id].transmem:
            ...
            if name in ['RHDF2_HUMAN_TRANSMEM1', 'PMP22_HUMAN_TRANSMEM3']:
                row.append(3)
                row.append(5)
            elif prob_notm >= noTMCutoff and prob_tm <= TMCutoff and type == 'Cytoplasmic':
                row.append(2)
                row.append(3)
...
#print (list(df["Highlight"] .to_numpy()))
#sys.exit()
fig=pylab.figure()
...
#df.to_csv('../data/part_c_data.tsv', sep='\t')
plt.savefig('../plots/figC.svg')
