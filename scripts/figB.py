...
map_start = {}
for line in open('../data/part_b_annotations.tsv', 'r'):
    if line[0] != '#':
        name = line.split('\t')[0]
...
l = 'ID\tGENE\tACC\tTM protein\tTotal_MUT\tUniProt MUT\tClinVar MUT\tCOSMIC MUT\tPosition\tSignalP_TM\tSignalP_noTM\tN-terminus\tTopology\n'
total_count = 0
count_excl = 0
count_type1 = 0
count_type2 = 0
...
for id in dic:
    if dic[id].signal in [0, 1]:
        ...
        for tm in dic[id].transmem:
            ...
            if name in ['RHDF2_HUMAN_TRANSMEM1', 'PMP22_HUMAN_TRANSMEM3']:
                row.append(2)
            elif prob_notm >= 0.5 and prob_tm <= 0.5 and type == 'Cytoplasmic':
                row.append(1)
            else:
                row.append(0)
            data.append(row)
...
df = pd.DataFrame(data, columns=['ID', 'GENE', 'ACC', 'Name', 'TMD', 'Start', 'End', 'Postiion', 'TM protein', 'SIGNALp', 'Total_MUT', 'Uniprot_MUT', 'ClinVar_MUT', 'COSMIC_MUT', 'Size', 'N-terminus', 'signalp_TM', 'signalp_noTM', 'Topfind', 'Highlight'])
...
ax = sns.scatterplot(data=df, x="signalp_noTM", y="signalp_TM", hue="Highlight", legend='brief')
...
ax.plot([0.5,0.5], [0.0,1.0], color='purple', linewidth=0.5, linestyle='--')
ax.plot([0.0,1.0], [0.45,0.45], color='purple', linewidth=0.5, linestyle='--')
ax.plot([0.0,1.0], [0.0,1.0], color='black', linewidth=0.5, linestyle='--')
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
#plt.savefig('../part_B_plots/fig_partb.eps')
#plt.savefig('../part_B_plots/fig_partb.svg')
#plt.savefig('../part_B_plots/fig_partb.png')
plt.savefig('../plots/figB.svg')
plt.savefig('../plots/figB.eps')
#df.to_csv('../data/part_b_data.tsv', sep='\t')
#df.to_csv('../data/part_b_data_try.tsv', sep='\t')
#plt.savefig('../part_B_plots/all_cytoplasmic.svg')
#plt.savefig('../part_B_plots/all_TM.svg')
#plt.savefig('../part_B_plots/all_cytoplasmic_TM.svg')
#plt.show()
