...
for id in dic:
    if dic[id].fasta != ''  and len(dic[id].fasta) >= 70:
        for tm in dic[id].transmem:
            ...
            m = '>'+name+'\n'+dic[id].fasta[start_seq:end_seq]
            l += m + '\n'
            if os.path.isfile('../data/part_c_fasta/'+name+'.fasta') == False:
                open('../data/part_c_fasta/'+name+'.fasta', 'w').write(m)
            z += name + '\t' + str(start) + '\t' + str(start_seq+70) + '\t' + str(start) + '\t' +  str(end) + '\n'
            if os.path.isfile('../data/part_c_signalp/'+name+'_long_TM.out') == False:
                print (name)
                os.system('/home/gurdeep/Downloads/signalp-4.1/signalp_TM -f long -s best ../data/part_c_fasta/'+name+'.fasta > ../data/part_c_signalp/'+name+'_long_TM.out')
            if os.path.isfile('../data/part_c_signalp/'+name+'_long_noTM.out') == False:
                print (name)
                os.system('/home/gurdeep/Downloads/signalp-4.1/signalp_noTM -f long -s best ../data/part_c_fasta/'+name+'.fasta > ../data/part_c_signalp/'+name+'_long_noTM.out')
            #sys.exit()
...
open('../data/part_c_annotations.tsv', 'w').write(z)
