...
for id in dic:
    if dic[id].signal in [0, 1]:
        ...
        for tm in dic[id].transmem:
            ...
            if prob_notm >= 0.5 and prob_tm <= 0.55:
                row.append('YES')
            else:
                row.append('NO')
            data.append(row)
...
df = pd.DataFrame(data, columns=['ID', 'GENE', 'ACC', 'Name', 'TMD', 'Start', 'End', 'Position', 'TM protein', 'Signalp', 'Total_MUT', 'UniProt_MUT', 'ClinVar_MUT', 'COSMIC_MUT', 'Size', 'N_terminus', 'SignalP_TM', 'SignalP_noTM', 'Topfind', 'Highlight'])
...
df.to_csv('../data/part_b_data.tsv', sep='\t', index= False)
